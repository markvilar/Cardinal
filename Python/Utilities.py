...
def clamp_signal(data: np.ndarray, min_value: float, max_value: float):
    ...
    adjustments[upper_mask] = np.ceil(data[upper_mask] / (max_value - min_value)) \
        * (max_value - min_value)
    adjustments[lower_mask] = -np.floor(data[lower_mask] / (max_value - min_value)) \
        * (max_value - min_value)
    #data[upper_mask] = data[upper_mask] / (max_value - min_value)
    #print(mask)
    #divisions = data - min_value / (max_value - min_value)
    #divisions[divisions > 0] = np.floor(divisions[divisions > 0])
    #divisions[divisions < 0] = np.ceil(divisions[divisions < 0])
